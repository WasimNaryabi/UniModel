...
include("models/specific_student_transcript_backend.php"); 
...
/*echo $secrecy_id ." / ".$roll_number ." / ".$discipline ." / ".$semester ." / ".$session;exit(); OK*/
...
/* ====================================== Select Query for Retrival of Session Details from Database ==================================== */
        ...
        $select_semester =$pdo->prepare("select * from student as s, student_history as sh, semester as sem, discipline as d where s.roll_number = '$roll_number' and 
                            s.discipline_id = '$discipline' and s.session_id = '$session' and sh.semester_id = '$semester' and 
                            sem.semester_id = '$semester' and s.student_id = sh.student_id and s.discipline_id = d.discipline_id and 
                            d.discipline_id = '$discipline'");
        $select_semester->setFetchMode(PDO::FETCH_ASSOC);
        $select_semester->execute();
        $show = $select_semester->fetch();
         /*print_r($show);exit();*/
            
            $student_name = $show['first_name']." ".$show['last_name'];
            $student_history_id = $show['student_history_id'];
            $registration_number = $show['uni_reg_number'];
            $discipline_name = $show['discipline_name'];
            $semester_name = $show['semester_name'];
            $father_name = $show['father_name'];
            $roll_number = $show['roll_number'];
            $student_id = $show['student_id'];
            $gender = $show['gender'];

            /*echo $student_name." / ".$student_history_id." / ".$registration_number." / ".$discipline_name." / ".$semester_name." / ".$father_name." / ".$roll_number." / ".$student_id." / ".$gender." / ";  
            exit()
            ;working*/
/* ====================================== Query for Retrival of Student Transcript ===================================== */
    
        $student_result = $pdo->query("select c.course_id as 'course_id', c.course_name as 'course_name', c.credit_hours, sum(am.marks_obtained) as 'ass_marks', 
                         sum(am.total_marks) as 'ass_total', sum(qm.obtained_marks) as 'quiz_marks', sum(qm.total) as 'quiz_total', 
                         mid.midterm_marks_obtained as 'mid_marks', f.finalterm_marks_obtained as 'final_marks' from student as s,
                         courses as c, assignment_marks as am, quiz_marks as qm, midterm_result as mid, finalterm_result as f where 
                         s.discipline_id = '$discipline' and s.session_id = '$session' and c.course_id = am.course_id and c.course_id = qm.course_id and 
                         c.course_id = mid.course_id and c.course_id = f.course_id and s.student_id = '$student_id' and 
                         am.student_history_id = '$student_history_id' and qm.student_history_id = '$student_history_id' and 
                         mid.student_history_id = '$student_history_id' and f.student_history_id = '$student_history_id' and c.semester_id = '$semester' 
                         and c.discipline_id = '$discipline' group by am.course_id order by c.serial_no");
        $total_marks_obtained = 0;
        $total_marks =0 ;
        $total_crhr = 0;
        $total_gpa = 0.00;

        /*print_r($student_result);exit();*/
...
?>
            ...
            <td>Univ. Reg No : <?php echo $registration_number; ?></td>
            <td>&nbsp;</td>
        </tr>
        <tr>
            <td colspan="3">
